...
# Docker quick-start: .admin_credentials is mode 0600 and owned by sova (UID 10001).
# docker compose exec runs as root, but cap_drop:ALL removes CAP_DAC_OVERRIDE, so root
# cannot read the file. Re-exec as sova when invoked as root (issue #195).
set -e
...
if [ "$(id -u)" = "0" ]; then
    ...
    exec su -s /bin/sh sova -c 'exec "$0" "$@"' -- "$0" "$@"
fi
